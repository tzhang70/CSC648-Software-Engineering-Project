...
require_once "config.php";
    $result = mysqli_query($link,"SELECT * FROM users_list where username = '".$_GET["name"]."'");
    ...
    if(isset($_POST["submit"]) == "submit" && isset($_POST["eventNotes"]) != "")
    {
      $sql = "INSERT INTO appointments_list (notes, start_event, student_id, professor_id, end_event, student_name, professor_name)
        VALUES ('".$_POST['eventNotes']."', '".$_POST['eventDate']."', '".$_SESSION['id']."', '".$_POST['profID']."', '".$_POST['eventEndDate']."','".$_SESSION['name']."', '".$_POST['profName']."')";
    ...
    $sql = "SELECT appointment_id as id, notes as description, start_event as start, end_event as end, busy as title FROM appointments_list WHERE professor_id='".$profID."'";
        ...
        document.addEventListener('DOMContentLoaded', function() {
          ...
          var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            dateClick: function(info) {
                                                   ...
                                                   var startDate = moment(info.date).subtract(480, 'm').toDate();
                                                   $("#eventDate").val(startDate.toISOString().slice(0, 19).replace('T', 'T'));
                                                   var endDate = moment(info.date).subtract(450, 'm').toDate();
                                                $("#eventEndDate").val(endDate.toISOString().slice(0, 19).replace('T', 'T'));
                                                   }
              //alert('Clicked on: ' + info.dateStr);
         ...
         <div class="row">
            ...
            <div class="col-xl-9 col-lg-9 col-md-9 col-sm-9 col-9 calendar" style='font-size:17px'>
               <div class="modal fade" id="successModal" role="dialog" aria-labelledby="successModalLabel" aria-hidden="true">
                     ...
                     <input type="hidden" name="profID" class="form-control" id="profID" value="<?php echo htmlspecialchars($profID); ?>"
>
                     <input type="hidden" name="profName" class="form-control" id="profName" value="<?php echo htmlspecialchars($profName); ?>"
>
